...
class If_Else_AST:
    def __init__(self, condition, then, elze):
        self.condition = condition
        self.then = then
        self.elze = elze
    def __repr__(self):
...
def write():
    ...
    return Write_AST(next_item)

def read():
    """Reads shtuff"""
    scanner.consume(Token.READ)
    next_item = identifier()
    return Read_AST(next_item)
...
def if_statement():
    ...
    then = statements()
    next_item = scanner.lookahead()
    if next_item == (Token.END):
        scanner.consume(Token.END)
        #print('if')
        return If_AST(condition, then)
        
    elif next_item == (Token.ELSE):
        #print('elif')
        scanner.consume(Token.ELSE)
        elze = statements()
        scanner.consume(Token.END)
        return If_Else_AST(condition, then, elze)
    else:
